...
const TeacherSection = ({ data }) => {
  // Default data from user request for fallback/dev
  const defaultData = {
    markingScheme: {
      segments: [
        {
  ...
  const { markingScheme } = currentData;
    ...
    <div className="w-full max-w-5xl mx-auto p-4 md:p-6 space-y-8 bg-gray-50/50 min-h-screen">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mb-6">
        ...
        <p className="text-gray-500 mt-1">
          Detailed breakdown of teacher expectations vs student performance
        </p>
      </div>
                  ...
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span
                      className={`text-sm font-bold whitespace-nowrap ml-2 ${
                        segment.awardedMarks === segment.maxMarks
                          ? "text-emerald-600"
                          : "text-amber-500"
                      }`}
                    >
                      Awarded: {segment.awardedMarks}
                    </span>
                  </div>
        ...
        <div className="md:col-span-2 bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2">
              {markingScheme.deductions.marksDeducted > 0 && (
                <span className="px-3 py-1 bg-red-50 text-red-600 font-semibold text-sm rounded-lg border border-red-100">
                  -{markingScheme.deductions.marksDeducted} Marks
                </span>
              )}
            </div>
          </div>
      ...
      {/* Final Score (Bottom) */}
      <div className="bg-gray-900 rounded-2xl p-8 flex flex-col md:flex-row justify-between items-center text-white shadow-xl mt-8">
        <div>
          <h3 className="text-xl font-bold mb-1">Final Result</h3>
          <p className="text-gray-400 text-sm">
            Total marks awarded after all deductions
          </p>
        </div>
